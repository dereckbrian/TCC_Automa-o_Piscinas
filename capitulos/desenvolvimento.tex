...
\section{Fase de contrução (\textit{Construction})}
    ...
    % --- PARÁGRAFO NOVO (Atendendo ao seu comentário de Testes) ---
    Paralelamente à codificação, foram executados testes unitários e de integração. Foi realizada a calibração do sensor de temperatura comparando-o com um termômetro de mercúrio padrão para ajustar o \textit{offset} no código. Além disso, foram conduzidos testes de estresse no acionamento da bomba para validar a resposta do sistema frente a variações rápidas no nível da água, garantindo a confiabilidade do atuador antes da implantação final.
    
    % --- REFERÊNCIA À DOCUMENTAÇÃO ---
    A documentação técnica, incluindo os diagramas de classe, esquemáticos de ligação e trechos detalhados do código-fonte comentado, encontra-se detalhada na seção de Resultados e no Apêndice A deste trabalho.
