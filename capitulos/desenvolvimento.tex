...
\section{Fase de contrução (\textit{Construction})}
    ...
    Durante essa fase, foi feita toda a parte de implementação, pegando sensores que antes estavam separados e junta-los no arduíno e fazendo com que alguns trabalhassem juntos. Primeiro foi feita a implementação e codificação do sensor de temperatura, logo a pós, foi introduzido o sensor de nível junto a bomba submersa, para que os dois trabalhassem juntos, seguindo uma lógica dê: Caso o nível da água desça, o sensor de nível manda um alerta pra bomba ligar, quando encher o sensor de nível sobre e avisa a bomba para desligar.

    Todo o código do funcionamento dos sensores e atuadores foi desenvolvido em uma IDE própria para o arduíno que lida muito bem com a linguagem C like que o arduíno reconhece. O arduíno é conectado é uma das portas USB do raspberry, que, por um código python absorve as informações dos sensores e atuadores do arduíno e por meio de uma requisição HTTP com o back-end, ele envia as informações para serem salvas no banco de exibidas no front-end. A seção subsequente detalhará melhor o funcionamento do código.

    As linguagens utilizadas nesse projeto, foram escolhidas por sua curva de aprendizagem rápida. Foram escolhidas React como framework front-end e SpringBoot como framework back-end. A aplicação funciona de forma que o front faz requisições para o back-end via HTTP, e o back-end responde com a informação solicitada. Em um contexto prático, funcionaria da seguinte forma: A temperatura é enviada ao back-end e salva no banco de dados a cada 5 minutos, dessa forma, algum card no back-end que fica responsável por mostrar essa informação, faz constantes requisições para o back-end fornecer sempre a ultima temperatura coletada, e o próprio responde com a tal.
