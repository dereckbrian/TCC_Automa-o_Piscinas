...
Na camada de aplicação, o \textit{back-end}, desenvolvido utilizando o \textit{framework} Spring Boot, centraliza a lógica do sistema e disponibiliza os \textit{endpoints} para a recepção e persistência dos dados. Uma funcionalidade fundamental desenvolvido nesta etapa foi o algoritmo de recomendação de dosagem química, capaz de sugerir correções precisas (ex: quantidade em gramas de elevador de pH) com base nos parâmetros monitorados. Para viabilizar esses cálculos, o sistema exige a parametrização do volume da piscina (em litros) nas configurações do usuário. Esse valor é armazenado no banco de dados e utilizado como constante multiplicadora nas fórmulas de tratamento. O \textit{controller} responsável por mapear as requisições, converter os objetos JSON e armazenar as leituras no banco de dados PostgreSQL está apresentado no Apêndice \ref{apendice:codigos}, Seção \ref{cod:java}. Essa arquitetura assegura a integridade, a consistência e a disponibilidade do histórico de monitoramento.
