...
O monitoramento do nível da água é realizado pelo sensor LC26M-40, apresentado na Figura \ref{fig:sensorlevel} (Apêndice \ref{apendice:hardware}). Fabricado em polipropileno, seu funcionamento baseia-se em um interruptor magnético (\textit{reed switch}) acionado pelo deslocamento de um flutuador, fornecendo um sinal digital (SPST)\footnote{Refere-se a um tipo de interruptor (chave). A sigla significa \textit{Single-Pole}, \textit{Single-Throw} (unipolar, de uma via ou um estágio), descrevendo o mecanismo físico de um interruptor simples de ligar/desligar (\textit{on/off}). } para identificar níveis críticos no reservatório.
