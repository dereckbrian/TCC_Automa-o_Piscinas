...
\section{Fase de Elaboração (\textit{Elaboration})}
    ...
    Além disso, o sistema conta com um front-end que permite ao usuário acompanhar os dados coletados pelos sensores. A interface oferece, por exemplo, gráficos que exibem a média de temperatura diária ou cartões que são atualizados automaticamente conforme novas leituras são realizadas. O usuário também pode enviar comandos pelo front-end, como: ligar a bomba d’água, acionar o filtro, ativar o aquecedor ou a cascata.
    ...
    Para complementar a compreensão do funcionamento do sistema, será inserido um fluxograma do fluxo de dados, ilustrando desde a coleta pelo Arduino até a comunicação final com o back-end e a visualização pelo usuário.
    ...
    Foto do sensor:
    Este sensor é responsável por medir a temperatura da água da piscina. Ele é conectado ao Arduino, que realiza a leitura periódica e envia o valor obtido ao Raspberry Pi, que repassa a informação ao back-end.

    \subsection*{Sensor de Nível}

    Foto do sensor:
    O sensor de nível detecta se a água está acima ou abaixo do limite desejado. Assim como o sensor de temperatura, ele transmite sua leitura ao Arduino, que a encaminha ao Raspberry para processamento e registro.
    ...
    \subsection*{Bombas Submersas (5 cv)}
