...
\begin{table}[H]
    ...
    \begin{tabular}{|p{3.5cm}|p{5.0cm}|c|p{3.5cm}|}
        \hline
        \textbf{Componente} & \textbf{Função Principal} & \textbf{Tensão} & \textbf{Comunicação / Sinal} \\ \hline
        Raspberry Pi 3 B & Gateway de comunicação e envio de dados ao servidor & 5V & Serial (USB) / Wi-Fi (HTTP) \\ \hline
        Arduino Uno R3 & Microcontrolador para leitura de sensores e controle de atuadores & 5V / 9V & Serial / I/O Digital e Analógico \\ \hline
        Sensor Temp. (NTC) & Monitoramento térmico da água & 5V & Analógico (Divisor de Tensão) \\ \hline
        Sensor de Nível & Detecção de nível crítico do reservatório & 5V & Digital (On/Off) \\ \hline
        Sensor de pH & Medição da acidez e alcalinidade & 5V & Analógico \\ \hline
        Sensor de Turbidez & Análise da transparência da água & 5V & Analógico \\ \hline
        Bomba Submersa & Atuador para circulação e filtragem & 3V - 6V & Acionamento via Relé (Sinal Digital) \\ \hline
    \end{tabular}
    \caption*{Fonte: Autoria própria.}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{4.0cm}|p{6.0cm}|p{3.5cm}|}
        \hline
        \textbf{Teste Realizado} & \textbf{Objetivo do Teste} & \textbf{Resultado} \\ \hline
        Calibração do Sensor NTC & Verificar a precisão térmica comparada a um termômetro de referência & \\ \hline
        Calibração do Sensor de pH & Verificar a precisão do sensor comparando com testes de estojos para analisar parâmentros da água & \\ \hline
        Validação de Turbidez & Verificar linearidade e sensibilidade através do método de diluição seriada (água límpida vs. saturada) & \\ \hline
        Estabilidade do Sensor de Nível & Confirmar acionamento correto da boia sem oscilações espúrias (debounce) &  \\ \hline
        Proteção \textit{Dry-run} & Impedir o acionamento da bomba quando o reservatório está vazio &  \\ \hline
        Integração de Dados & Validar o fluxo: Arduino $\to$ Raspberry $\to$ Banco de Dados &  \\ \hline
        Latência da Interface & Medir tempo entre a leitura do sensor e atualização na tela &  \\ \hline
    \end{tabular}
    \caption*{Fonte: Autoria própria.}
\end{table}
