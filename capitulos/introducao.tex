...
Durante a pesquisa bibliográfica, foram identificados diferentes tipos de trabalhos, com diferentes propostas tecnológicas, variando desde acionamentos via temporizadores até sistemas baseados em controladores lógicos industriais. A seguir, serão apresentadas essas diferentes abordagens para fins comparativos.
...
\begin{table}[H]
    ...
    \textbf{\citeonline{campos2014automaccao}} & Automação residencial geral. Controle da piscina por \textit{timer}. & Tomada de decisão baseada em sensores (pH e Turbidez) e não apenas tempo. & O correlato possui escopo mais amplo (integra portão, luzes da casa, etc). \\ \hline
    \textbf{\citeonline{brandao2018sistema}} & Controle químico (pH e ORP) em malha fechada com Arduino. & Monitoramento de Turbidez para limpeza física e conectividade IoT (Web) para histórico. & O correlato utiliza sensor de ORP para leitura de cloro, parâmetro não abordado neste trabalho. \\ \hline
    \textbf{\citeonline{boeira2019autopool}} & Dosagem química automatizada e sistema de alarme de segurança. & Interface Web para gestão remota e sensor de turbidez para controle do filtro. & O correlato possui sistema de segurança contra afogamentos. \\ \hline
    \textbf{\citeonline{sulimann2014automatizacao}} & Automação industrial com CLP, aquecimento e ionização. & Baixo custo de implementação (Open Hardware) e facilidade de integração IoT. & O correlato apresenta maior robustez industrial e controle de temperatura. \\ \hline
    \textbf{\citeonline{oliveira2023automaccao}} & Acionamento remoto via Wi-Fi (ESP32) e infraestrutura elétrica. & Autonomia do sistema (não depende do usuário para ligar) baseada em dados analíticos. & O correlato foca no dimensionamento elétrico de potência (contatores). \\ \hline
    \textbf{Este Trabalho} & Sistema IoT completo com monitoramento de pH e Turbidez e interface Web. & Integração entre sensoriamento físico-químico, lógica autônoma e gestão remota de dados. & Validação em escala de protótipo, necessitando adaptações para altas potências. \\ \hline
    \end{tabular}%
    }
    \par \vspace{0.2cm}
